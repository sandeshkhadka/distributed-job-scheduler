...
The functional requirements describe the specific behaviors and capabilities that the system must provide, as illustrated in the use case diagram (Figure~\ref{fig:use-case}). Detailed use case descriptions follow.

\begin{figure}[htbp]
	\centering
	\figimg{figures/use-case.png}
	\caption{Use case diagram showing actors and use cases for the Distributed Job Scheduler}
	\label{fig:use-case}
\end{figure}

\begin{description}
	\item[Actor 1: Client (User)] A user who submits jobs to the system.
...
\begin{enumerate}
	...
	\item \textbf{Get Job Status:} A client queries the status of a previously submitted job. The system returns the current status (e.g., \texttt{"not started"}, \texttt{"assigned"}, \texttt{"completed"}, \texttt{"failed"}).
...
The class diagrams presented below show the main classes identified during analysis, grouped by subsystem for legibility.

\begin{figure}[htbp]
	\centering
	\figimg{figures/class-analysis-scheduler.png}
	\caption{Analysis-level class diagram: Scheduler-side classes}
	\label{fig:class-analysis-scheduler}
\end{figure}

The scheduler-side classes identified during analysis (Figure~\ref{fig:class-analysis-scheduler}) are:
...
\begin{description}
	...
	\item[JobSelectorRegistry:] Factory class that manages the registration and creation of selector implementations.
\end{description}

\begin{figure}[htbp]
	\centering
	\figimg{figures/class-analysis-worker.png}
	\caption{Analysis-level class diagram: Worker-side classes}
	\label{fig:class-analysis-worker}
\end{figure}

The worker-side classes identified during analysis (Figure~\ref{fig:class-analysis-worker}) are:
...
\begin{description}
	...
	\item[JobExecutor (abstract):] Defines the interface for different executor types. Concrete implementations include StressCpuExecutor, StressMemExecutor, StressIOExecutor, and MixedLoadExecutor, with additional types addable through the same interface.
	...
	\item[ExecutorRegistry:] Factory class that manages the registration and creation of executor implementations.
\end{description}
...
\begin{figure}[htbp]
	\centering
	\figimg{figures/sequence-job-lifecycle.png}
	\caption{Sequence diagram showing the complete job lifecycle from submission to completion}
	\label{fig:sequence-job-lifecycle}
\end{figure}

The job lifecycle sequence (Figure~\ref{fig:sequence-job-lifecycle}) proceeds as follows:
\begin{enumerate}
	...
	\item The Worker periodically sends \texttt{GetJob} requests. The Scheduler's \texttt{JobSelector} selects the most appropriate job and assigns it, updating the status to \texttt{"assigned"}.
	...
	\item The \texttt{EbpfMonitor} reads BPF maps and the \texttt{EbpfMetricStore} buffers snapshots, which are periodically sent via \url{ReportJobEbpfMetrics}.
	\item Upon job completion, the \texttt{JobOrchestrator} parses the executor output and invokes the \url{on_completed} callback, which sends \texttt{ReportJobResult} to the Scheduler.
	\item The Scheduler saves the result and computes runtime analytics.
\end{enumerate}

\begin{figure}[htbp]
	\centering
	\figimg{figures/sequence-worker-registration.png}
	\caption{Sequence diagram showing the worker registration process}
	\label{fig:sequence-worker-registration}
\end{figure}

The worker registration sequence (Figure~\ref{fig:sequence-worker-registration}) proceeds as follows:
\begin{enumerate}
...
\begin{figure}[htbp]
	\centering
	\figimg{figures/sequence-metrics-reporting.png}
	\caption{Sequence diagram showing the periodic metrics reporting flow}
	\label{fig:sequence-metrics-reporting}
\end{figure}

\subsubsection{State Diagram}

The state diagram (Figure~\ref{fig:state-job-states}) captures the states and transitions of a job throughout its lifecycle.

\begin{figure}[htbp]
	\centering
	\figimg{figures/state-job-states.png}
	\caption{State diagram showing job states and transitions}
	\label{fig:state-job-states}
\end{figure}
...
\begin{description}
	...
	\item[assigned:] A worker has been assigned to the job, but the worker has not yet confirmed receipt.
...
The activity diagram (Figure~\ref{fig:activity-workflow}) models the workflow of the job scheduling and execution process.

\begin{figure}[htbp]
	\centering
	\figimg{figures/activity-workflow.png}
	\caption{Activity diagram showing the scheduling and execution workflow}
...
\begin{enumerate}
	...
	\item The Executor runs the job; the eBPF monitor collects kernel-level metrics.
