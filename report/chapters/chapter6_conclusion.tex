...
\begin{enumerate}
	\item \textbf{Centralized Job Management:} The gRPC-based scheduler provides a single point for job submission, status tracking, and result collection. The service definition, comprising 12 RPC methods, covers the complete job lifecycle from registration through execution to analytics.
	...
	\item \textbf{Kernel-Level Monitoring:} The custom eBPF program, attached to the \url{raw_syscalls/sys_enter} tracepoint, provides real-time monitoring of system call activity, I/O operations, and network transfers at the kernel level. This represents a significant improvement over traditional process-level monitoring in terms of granularity and accuracy.
...
\begin{enumerate}
	...
	\item \textbf{ML-Driven Scheduling:} Train a compact and lightweight machine learning model from the historical job and worker data acquired by the system. When deployed on the scheduler as a scheduling algorithm, such a model can make faster and more accurate scheduling decisions by learning from past execution patterns, worker performance profiles, and resource utilization trends, going beyond the heuristic-based approach of the current FCFS, SJF, and Adaptive selectors.
\end{enumerate}
